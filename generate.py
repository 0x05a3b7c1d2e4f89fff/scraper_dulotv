...
# Full browser-grade headers — avoids Cloudflare 403 on GitHub Actions IPs
HEADERS = {
    "User-Agent": (
...
def fetch_channels() -> list[dict]:
    ...
    # Use a Session so cookies from the initial page load carry over
    session = requests.Session()
    session.headers.update(HEADERS)

    # Hit the homepage first to pick up any Cloudflare cookies
    try:
        session.get("https://dulo.tv/", timeout=15)
        time.sleep(1)
    except Exception:
    ...
    if r.status_code == 403:
        print(f"  [error] 403 from dulo.tv — Cloudflare is blocking the runner IP.")
